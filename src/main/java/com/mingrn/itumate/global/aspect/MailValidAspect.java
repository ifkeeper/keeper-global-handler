...
@Order(5)
@Component
public class MailValidAspect {

	private static final Logger LOGGER = LoggerFactory.getLogger(MailValidAspect.class);
	...
	public void beforeNotice(JoinPoint joinPoint) {
		...
		for (Parameter parameter : parameters) {
			...
			if (parameter.getType().isAssignableFrom(HttpServletRequest.class) || parameter.getType().isAssignableFrom(HttpSession.class) ||
					parameter.getType().isAssignableFrom(HttpServletResponse.class) || parameter.getAnnotation(MailValid.class) == null) {
				continue;
			}
			// 参数是否增加不为NULL 或 "" 条件
			MailValid mailValid = parameter.getAnnotation(MailValid.class);
			if (!mailValid.valid()) {
				continue;
			}
			...
			String mails = request.getParameter(paramsName);
			String[] mail = mails.split(",");
			for (String s : mail) {
				while (!MailValid.REGEXP.matcher(s).find()) {
					throw new RuntimeException(mailValid.message());
				}
			}
			LOGGER.info("Checked [{}] parameter [{}], and the value is: [{}]", clazz.toString(), paramsName, mails);
		}
